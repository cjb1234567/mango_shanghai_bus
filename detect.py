...
# 公交车提醒地址
BUS_ALARM_URL = 'http://192.168.43.180:9090/bus/get_audio/text'
#BUS_ALARM_URL = 'http://127.0.0.1:9090/bus/get_audio/text'
...
def text2mp3(text, voicefile):
    ...
    if not isinstance(res, dict):
        with open('music/'+voicefile+'.mp3', 'wb') as f:
            for chunk in res.iter_content(chunk_size=1024):
                if chunk:
                    f.write(chunk)
    return 'music/'+voicefile+'.mp3'
...
def alarm_timer():
    ...
    while True:
        ...
        print(alarm_rs)
        if alarm_rs != '':
            alarm_mp3 = text2mp3(alarm_rs, 'alarmvoice')
            print(alarm_mp3+' created')
            play_mp3(alarm_mp3)
        time.sleep(30)
...
if __name__ == "__main__":
    ...
    for phrase in speech:
        ...
        answer_mp3 = text2mp3(answer_text, 'answervoice')
